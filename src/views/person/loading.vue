...
export default {
  ...
  methods: {
    ...
    async getStatus () {
      const result = await post('/name_selfmarriage/order_result', { order_id: this.orderidPerson })
      ...
      if (result.result === 0) {
        ...
        this.over = true
        this.$router.replace({ path: '/person/order_detail' })
      } else {
...
.loading-box {
    width: 100vw;
    height: 100vh;
    background: linear-gradient(180.00deg, rgb(238, 162, 165),rgb(250, 234, 223) 100%);
    position: relative;
